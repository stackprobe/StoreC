...
namespace Charlotte.GameTools
{
	public class SimpleMenu
	{
		...
		public void PadConfig(bool keyMode = false)
		{
			...
			try
			{
				if (keyMode)
				{
					...
					foreach (ButtonInfo btnInfo in btnInfos)
						btnInfo.Button.KeyIds = new int[0];
					...
					while (currBtnIndex < btnInfos.Length)
					{
...

						{
							...
							for (int c = 0; c < currBtnIndex; c++)
								if (btnInfos[c].Button.KeyIds.Any(keyId => keyId == pressKeyId))
									pressKeyId = -1;
							...
							if (pressKeyId != -1)
							{
								...
								btnInfos[currBtnIndex].Button.KeyIds = keyIds;
								currBtnIndex++;
							}
						}
					endInput:
						...
						for (int c = 0; c < btnInfos.Length; c++)
						{
							...
							if (c < currBtnIndex)
							{
								int[] keyIds = btnInfos[c].Button.KeyIds;
								...
								if (keyIds.Length == 0)
									...
									DDPrint.Print(string.Join(", ", keyIds.Select(keyId => KeyInfos.First(keyInfo => keyInfo.KeyId == keyId).Name)));
							}
							DDPrint.PrintRet();
						}
				...
				{
					foreach (ButtonInfo btnInfo in btnInfos)
						btnInfo.Button.BtnIds = new int[0];
					...
					while (currBtnIndex < btnInfos.Length)
					{
...

						{
							...
							for (int c = 0; c < currBtnIndex; c++)
								if (btnInfos[c].Button.BtnIds.Any(btnId => btnId == pressBtnId))
									pressBtnId = -1;

							if (pressBtnId != -1)
							{
								btnInfos[currBtnIndex].Button.BtnIds = new int[] { pressBtnId };
								currBtnIndex++;
							}
						}
					endInput:
						...
						for (int c = 0; c < btnInfos.Length; c++)
						{
							...
							if (c < currBtnIndex)
							{
								int[] btnIds = btnInfos[c].Button.BtnIds;
								...
								if (btnIds.Length == 0)
									...
									DDPrint.Print(string.Join(", ", btnIds.Select(btnId => "" + btnId)));
							}
							DDPrint.PrintRet();
						}
		...
		public void WindowSizeConfig()
		{
			...
			{
				string.Format("{0} x {1} (デフォルト)", WinSzExp(DDConsts.Screen_W, 0), WinSzExp(DDConsts.Screen_H, 0)),
