	...
	for (; ; )
	{
...

		{
			...
			for (; ; )
			{
				...
				var<double> rx = chinkoX - rotCtrX; // ちんこ相対(回転軸⇒描画位置)_X
				var<double> ry = chinkoY - rotCtrY; // ちんこ相対(回転軸⇒描画位置)_Y
				var<double> r = GetAngle(rx, ry);
				var<double> d = GetDistance(rx, ry);

				r += rot; // ちんこ回転適用

				var<D2Point_t> rPt = AngleToPoint(r, d); // 回転を適用したちんこ描画相対位置
				rx = rPt.X;
				ry = rPt.Y;

				var<double> x = rotCtrX + rx; // ちんこ描画位置_X
				var<double> y = rotCtrY + ry; // ちんこ描画位置_Y
